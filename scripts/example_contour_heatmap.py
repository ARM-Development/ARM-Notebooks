...
lwp = lwp[np.logical_and(time_lwp>=time1, time_lwp<=time2)]
...
fig,ax = plot.heatmap([ccn2,ccn2_m], [lwp,lwp_m], [ndrop,nd_m], 
                    xedges=np.arange(0,1000,100), yedges=np.arange(0,510,50),
                    xlabel='0.2%CCN (cm-3)', ylabel='LWP (g/m2)', zlabel='cm$^{-3}$',
                   title=['Obs','E3SMv2'])
# ax.set_xscale('log')
fig.savefig(figpath+'heatmap_CCN2_LWP_Nd_sfc_'+site+'_'+IOP+'.png',dpi=fig.dpi,bbox_inches='tight', pad_inches=1)
